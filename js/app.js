...
function showScores(){
  var gameOver = "<h2 id='result'>Итоги</h2>";
  //quiz.score - кол-во правильных ответов
  gameOver += '<h2>Ваш результат: ' + Quiz.score + ' из ' + Quiz.questions.length + ' вопросов.</h2>'

  //Прописываем возможные случаи
  if (Quiz.score < 4) {
    gameOver += '<h3>Ознакомьтесь со всей информацией на нашем сайте и пройдите тест еще раз!<br>Желаем удачи!</h3>';
  }
  if (Quiz.score >= 4 && Quiz.score <= 6) {
    gameOver += '<h3>Неплохой результат, но могло быть и лучше.<br> Ознакомьтесь со всей информацией на нашем сайте и пройдите тест еще раз! </h3>';
  }
  if (Quiz.score > 6) {
    gameOver += '<h3>Поздравляем!<br> Вы отлично разбираетесь в тематике Олимпийских игр!</h3>';
  }
